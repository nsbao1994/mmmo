    ...
    def do_GET(self):
        try:
            ...
            if articles_ref:
                ...
                # Đã sửa lại lỗi thụt lề ở dòng này, thẳng hàng với vòng lặp for
                if valid_products:
                    promo = random.choice(valid_products)
                    # ÉP HTML THÀNH 1 DÒNG ĐỂ TRÁNH LỖI PRE-WRAP CỦA CSS
                    promo_html = f"""<a href="{promo['link']}" target="_blank" style="text-decoration: none; color: inherit; display: block; margin-top: 20px;"><div class="shopee-box" style="display: flex; align-items: center; background: #fff; border: 2px dashed #ee4d2d; border-radius: 8px; padding: 15px; gap: 20px; box-shadow: 0 4px 10px rgba(238,77,45,0.08);"><div class="shopee-box-img" style="width: 120px; height: 120px; flex-shrink: 0; border-radius: 6px; overflow: hidden; border: 1px solid #eee;"><img src="{promo['image']}" alt="Sản phẩm" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.src='https://picsum.photos/300/300'"></div><div class="shopee-box-info" style="flex-grow: 1; text-align: left;"><div class="shopee-box-title" style="font-weight: bold; font-size: 1.1em; color: #333; margin-bottom: 8px; line-height: 1.4;">🛒 {promo['name']}</div><div class="shopee-box-price" style="color: #555; font-size: 0.95em; margin-bottom: 5px;">Giá ưu đãi: <span style="color: #ee4d2d; font-weight: bold; font-size: 1.2em;">{promo['price']}</span></div><div class="shopee-box-desc" style="color: #26aa99; font-size: 0.85em; font-weight: bold; margin-bottom: 15px;">🔥 Đang có Mã Giảm Giá & Freeship Extra</div><span class="shopee-box-btn" style="display: inline-block; background: #ee4d2d; color: white; padding: 8px 20px; border-radius: 4px; font-weight: bold;">Đến Nơi Bán &rarr;</span></div></div></a>"""
            ...
            for attempt in range(3):
                try:
                    # Sử dụng đúng phiên bản gemini-3.6-flash của bạn
